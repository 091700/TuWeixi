...
public String discardIngredient(@PathVariable Integer id) {
    ...
    public String checkRecipe(@RequestBody Map<String, List<Integer>> payload) {
        ...
        for (Integer id : ingredientIds) {
            IngredientDict dict = dictMapper.selectById(id);
            if (dict != null) {
                Map<String, String> item = new java.util.HashMap<>();
                item.put("name", dict.getName());
                item.put("category", dict.getCategory());
                ingredients.add(item);
            }
        }
        
        Double score = aiService.predictHarmony(ingredients); 
        
        if (score > 80) return "味觉和谐度：" + score + "分。绝妙搭配，快去大显身手！";
        if (score > 55) return "味觉和谐度：" + score + "分。中规中矩，可以一试。";
        if (score > 35) return "味觉和谐度：" + score + "分。勉强能吃，有点怪。";
        if (score == 0.0) return "味觉和谐度：0分。💥 严重生化武器警告！不可食用！";
        return "味觉和谐度：" + score + "分。💥 暗黑料理预警！极度容易拉肚子！";
    }
    }